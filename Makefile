...
LIBEMU = $(OBJ)/libemu.bc
LIBOPTIONAL = $(OBJ)/$(TARGET)/$(SUBTARGET)/liboptional.bc
LIBBUS = $(OBJ)/$(TARGET)/$(SUBTARGET)/libbus.a
LIBDASM = $(OBJ)/$(TARGET)/$(SUBTARGET)/libdasm.bc
LIBUTIL = $(OBJ)/libutil.bc
LIBOCORE = $(OBJ)/libocore.bc
LIBOSD = $(OBJ)/libosd.bc
...
SOFTFLOAT = $(OBJ)/libsoftfloat.bc

# add formats emulation library
FORMATS_LIB = $(OBJ)/libformats.bc
...
ifeq ($(TARGETOS),win32)
	$(LD) $(LDFLAGS) $(LDFLAGSEMULATOR) $(VERSIONOBJ) $(EMULATOROBJ) $(LIBS) -lIphlpapi -lws2_32 -o $@
else
