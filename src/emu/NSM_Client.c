...
vector<boost::shared_ptr<MemoryBlock> > Client::createMemoryBlock(const std::string& name, unsigned char *ptr,int size)
{
  ...
  if(size>BYTES_IN_MB)
  {
    for(int a=0;; a+=BYTES_IN_MB)
    {
      ...
      {
        vector<boost::shared_ptr<MemoryBlock> > tmp = createMemoryBlock(name,ptr+a,size-a);
        retval.insert(retval.end(),tmp.begin(),tmp.end());
        break;
      }
      else
      {
        vector<boost::shared_ptr<MemoryBlock> > tmp = createMemoryBlock(name,ptr+a,BYTES_IN_MB);
        retval.insert(retval.end(),tmp.begin(),tmp.end());
      }
    }
    return retval;
  ...
  blocks.push_back(boost::shared_ptr<MemoryBlock>(new MemoryBlock(name,ptr,size)));
  staleBlocks.push_back(boost::shared_ptr<MemoryBlock>(new MemoryBlock(name,size)));
  syncCheckBlocks.push_back(boost::shared_ptr<MemoryBlock>(new MemoryBlock(name,size)));
