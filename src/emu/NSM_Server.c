...
bool Server::update(running_machine *machine) {
  ...
  RakNet::Packet *p;
  for (p = rakInterface->Receive(); p;
       rakInterface->DeallocatePacket(p), p = rakInterface->Receive()) {
...
void Server::sync(running_machine *machine) {
  ...
  if (syncCount > 0 && syncProcessor.get() == NULL) // FIXME not sure about null check
  {
    syncProcessor = std::shared_ptr<SyncProcessor>(new SyncProcessor(&syncProto, &syncPacketQueue,
