...
osd_dim sdl_window_info::pick_best_mode()
{
	...
	else
	{
		for (i = 0; i < num; ++i)
		{
			...
			size_score = 1.0f / (1.0f + std::abs((INT32)mode.w - target_width) + std::abs((INT32)mode.h - target_height));
			...
			if (m_win_config.refresh)
				size_score *= 1.0f / (1.0f + std::abs(m_win_config.refresh - mode.refresh_rate) / 10.0f);
